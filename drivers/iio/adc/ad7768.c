...
#define AD7768_4_REG_OFFSET(ch)		(AD7768_REG_OFFSET_BASE + (3 * __AD7768_4_REG_MAP(ch)))
#define AD7768_4_REG_GAIN(ch)		(AD7768_REG_GAIN_BASE + (3 * __AD7768_4_REG_MAP(ch)))
			    ...
			    int val, int val2, long info)
{
	...
	case IIO_CHAN_INFO_CALIBBIAS:
		...
		if (ret)
			goto unlock_write_bias;

		ret = regmap_write(st->regmap, base_reg + 1, AD7768_CALIB_REG_MID_MASK(val));
		if (ret)
			goto unlock_write_bias;

		ret = regmap_write(st->regmap, base_reg + 2, AD7768_CALIB_REG_LSB_MASK(val));
...
static void ad7768_set_available_sampl_freq(struct ad7768_state *st)
{
	...
	BUILD_BUG_ON(ARRAY_SIZE(ad7768_dec_rate) > MAX_FREQ_PER_MODE);
			       ...
			       struct device *dev)
{
	...
	ret = device_property_read_u32(&st->spi->dev, "adi,data-lines-number",
				       &st->datalines);
	if (ret) {
		dev_err(&st->spi->dev, "Missing \"adi,data-lines-number\" property\n");
		return ret;
	}

	ret = device_property_read_u32(&st->spi->dev, "adi,common-mode-output-v",
				       &vcm_sel);
	if (ret) {
	...
	for (i = 0; i < indio_dev->num_channels; i++) {
		...
		if (ret < 0) {
			dev_err(dev, "Failed to set sampling freq for channel %d\n",
				indio_dev->channels[i].channel);
			return ret;
		}
	}
	...
	dev_err(&st->spi->dev, "Invalid data-lines-number %d for %s\n",
...
static int ad7768_probe(struct spi_device *spi)
{
	...
	regmap_cfg = ad7768_regmap_config;
	if (st->chip_info->id == ID_AD7768)
		regmap_cfg.readable_reg = ad7768_readable_reg;
	else
		regmap_cfg.readable_reg = ad7768_4_readable_reg;

	st->regmap = devm_regmap_init(dev, &ad7768_regmap_bus, spi, &regmap_cfg);
	if (IS_ERR(st->regmap))
		return PTR_ERR(st->regmap);

	gpio_reset = devm_gpiod_get_optional(dev, "reset",
					     GPIOD_OUT_HIGH);
...
static int ad7768_runtime_suspend(struct device *dev)
{
	...
	mutex_lock(&st->lock);
	ret = regmap_write(st->regmap, AD7768_REG_POWER_MODE, val);
	mutex_unlock(&st->lock);
