...
static int adrv904x_probe(struct spi_device *spi)
{
	...
	phy->spi = spi;
	phy->chip_info = device_get_match_data(&spi->dev);
	phy->dev_clk = clk;
	phy->jdev = jdev;
	mutex_init(&phy->lock);
	...
	phy->kororDevice->common.deviceInfo.name = phy->chip_info->name;
...
static const struct adrv904x_chip_info adrv9040_chip_info = {
	.name = "ADRV9040",
};

static const struct of_device_id adrv904x_of_match[] = {
	{ .compatible = "adi,adrv9040", .data = &adrv9040_chip_info },
	{},
};
