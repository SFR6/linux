...
int __adrv903x_dev_err(struct adrv903x_rf_phy *phy, const char *function, int line);

#define adrv903x_api_call(phy, func, args...) ({				\
	int __ret = func((phy)->palauDevice, ##args);				\
	if (__ret)								\
		__ret = __adrv903x_dev_err(phy, __func__, __LINE__);		\
	__ret;									\
})
...
int adrv903x_register_debugfs(struct iio_dev *indio_dev);
#else
static inline int adrv903x_register_debugfs(struct iio_dev *indio_dev) { return 0; }
#endif
